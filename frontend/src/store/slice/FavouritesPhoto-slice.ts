...
const FavouritesPhotoSlice = createSlice({
    ...
    reducers: {
        ...
        removeFavouritesPhoto: (state, action: PayloadAction<ItemArray>) => {
            const index = state.value.findIndex(item => item.url === action.payload.url && item.text === action.payload.text);
            if (index !== -1) {
...
export const {setFavouritesPhoto, removeFavouritesPhoto} = FavouritesPhotoSlice.actions